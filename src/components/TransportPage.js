...
const TransportPage =() =>{
    ...
    const searchTransport = async () => {
        try {
            let response;
            if (searchType === "city") {
                response = await axios.post("http://localhost:8000/transport/getByCity", { city: searchQuery }, {
                    headers: {
                        Authorization: `Bearer ${token}`
                    }
                });
            } else if (searchType === "type") {
                response = await axios.post("http://localhost:8000/transport/getByType", { type: searchQuery }, {
                    headers: {
                        Authorization: `Bearer ${token}`
                    }
                });
